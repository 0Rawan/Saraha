...
export default function UserProfile() {
    ...
    const updateProfilePic = async (e) => {
        ...
        try {
          const response = await axios.post('https://sarahabackend.onrender.com/user/profilePic', formData , {
              headers: {
    ...
    <Modal
      ...
      >
        ...
        <Modal.Body>
                ...
                <Link className='user-link' to={`/user/${profileData?._id}`}>{"http://localhost:3000/user/"+profileData?._id}</Link>
                    ...
                    <div className='bg-white shadow-sm pb-3 rounded-2 overflow-hidden'>
                        ...
                        <div className='translate-middle-y'>
                            <div onClick={handleShow1} className='bg-black img-thumbnail img-square rounded-circle overflow-hidden mx-auto mouse-pointer'>
                                <img className='w-100 mb-0 rounded-circle' src={`https://sarahabackend.onrender.com/${profileData.profilePic}`} alt="" />
                            </div>
